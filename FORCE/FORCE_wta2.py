...
wf12 = uu[:,2:4]*.1
dw12 = dw*1
w12 = 2.0*(np.random.rand(N,2)-0.5)*.1#wo*1
...
#        Pt = np.array([np.exp(-(wo[:,0] @ r-ft[:,ti].sum())**2), np.exp(-(wo[:,1] @ r-ft[:,ti].sum())**2)]) #+ zk  #biased
        Pt = np.array([np.exp(w12[:,0]@r-e[0]**2), np.exp(w12[:,1]@r-e[1]**2) ])
#        Pt = np.array([np.exp(rt[:,t-1].T @ (np.outer(wf12[:,0] , dw12[:,0])) @ r -e[0]**2) + 0., \
#                       np.exp(rt[:,t-1].T @ (np.outer(wf12[:,1] , dw12[:,1])) @ r -e[1]**2)] )  # energy form!
...
for t in range(len(simtime)*1-1):
    ...
    x = (1.0-dt)*x + M @ (r*dt) + np.random.randn(N,1)*0.1*(wf12[:,0][:,None])
    r = np.tanh(x)
    z = wo.T @ r
    ...
    Pt_inf[:,ti] = ( np.array([np.exp(w12[:,0] @ r), np.exp(w12[:,1] @ r)])).squeeze()#/ \
#    sum(np.exp(wo[:,0] @ r) + np.exp(wo[:,1] @ r)) ).squeeze()
#    Pt_inf[:,ti] = np.array([np.exp(r.T @ (np.outer(wf12[:,0] , dw12[:,0])) @ r -1*e[0]**2) + 0., \
#                       np.exp(r.T @ (np.outer(wf12[:,1] , dw12[:,1])) @ r -1*e[1]**2)] ).squeeze()
