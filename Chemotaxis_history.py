...
def d_theta_hist(K_dcp, dc_perp, K, K_dc, K_dth, dC_t, dth_t):
    '''
    Return change in theta angle for each step
    Input with K_dcp for kernel, dc for orthogonal concentration difference dc_perp, and K covariance in weathervaning
    K_dc as the weighting/kernel on concentration dC_t in the signoidal function for tumbling rate 
    now adding then history for K_dth kernel acting on the angular history dth_t
    '''
    wv = np.dot(K_dcp,dc_perp) + K*np.random.randn()  #weathervaning strategy
    #P_event = 0.023/(0.4 + np.exp(40*dC_t/dt)) + 0.003  #sigmoidal function with parameters w
    P_event = 5*0.023/(1 + np.exp(np.dot(K_dc,dC_t/dt) + np.dot(K_dth, dth_t))) + 0.0  #less parameter version
...
K_dc = 20 *(tempk)+.0  #random-turning kernel (biphasic form, difference of two gammas)
...
for t in range(prehist,len(time)):
    ...
    dth_t = np.abs(np.diff( np.array([ths[t - past] for past in range(0,len(K_dcp)+1)]) ))
    dth = d_theta_hist(K_dcp, -dc_perp, K, K_dc, K_dth, dC, dth_t)
...
def nLL_hist(THETA, dtht,dcp,dc,dth):
    """
    negative log-likelihood objective function for fitting
    THETA includes parameter to be inferred and dtht, dcp, dc, dth are from recorded data
    """
    k_, A_, B_, Amp, tau, Amp_h, tau_h = THETA[0], THETA[1], THETA[2:7], THETA[7],THETA[8], THETA[9], THETA[10] 
    ### Kappa, A, Kdc, amp_dcp, tau_dcp, amp_hist, tau_hist
    ...
    D_ = Amp_h *np.exp(-K_win/tau_h)
    VM = rv.pdf((dtht-np.dot(dcp,C_))*d2r)
    P = sigmoid2(A_, B_, dc, D_, dth)
    marginalP = np.multiply((1-P), VM) + (1/(2*np.pi))*P
    nll = -np.sum(np.log(marginalP+1e-9))#, axis=1)
#    nll = -sp.special.logsumexp(marginalP)
    return nll#np.sum(nll)
...
def sigmoid2(a, b, x, d, dth):
    #a,b,c,d = p
    y = a / (1 + np.exp(np.dot(b,x.T/dt) + np.dot(d, dth.T)))
    ###Simulated function
...
def generate_noise(NN):
    ...
    for ii in range(NN):  #repetition
        ...
        dcs = np.zeros((time.shape[0],prehist))
        dcps = np.zeros((time.shape[0],prehist))
        dths = np.zeros((time.shape[0],prehist))
        dtht = np.zeros(time.shape)
        for t in range(prehist,len(time)):  #time series
            ...
            dth_t = (dtht[t-prehist:t]) #np.random.randn(len(K_dcp))*.10  #the actual angle change 
            dth = d_theta_hist(K_dcp, -dc_perp, K, K_dc, K_dth, dC, dth_t) 
            #data collection
            dcs[t] = dC  #concentration
            dcps[t] = dc_perp  #perpendicular concentration difference
            dths[t] = dth_t  #theta angle change history
            dtht[t] = dth  #one time point
    
        all_dc_p.append(dcps)  #recording dC_perpendicular
        all_dc.append(dcs)  #recording dC
        all_th.append(dtht)  #recording head angle
        all_dth.append(dths)
    ...
    data_dth = np.vstack(all_dth)
    
    return data_th, data_dcp, data_dc, data_dth

# %%
###generating data
data_th, data_dcp, data_dc, data_dth = generate_noise(20)


# %% with environment
# %%
#stimulate with white noise
def generate_taxis(NN):
    """
    Generate dth response with white noise mapping rather than concentration environment
    """
    all_dc_p = []
...
#            dth = d_theta_hist(K_dcp, -dc_perp, K, K_dc, K_dth, dC, dth_t) 
            ...
            dC = (dC-dC[-1])*5#/dt
            dc_perp = dc_perp - dc_perp[-1]
            
#            dth_t = np.abs(np.diff( np.array([ths[t - past] for past in range(0,len(K_dcp)+1)]) ))
            dth_t = (dtht[t-prehist:t])
            dth = d_theta_hist(K_dcp, -dc_perp, K, K_dc, K_dth, dC, dth_t)
            ths[t] = ths[t-1] + dth*dt
...
# %%
###generating data
data_th, data_dcp, data_dc, data_dth = generate_taxis(20)  

# %%
#optimize all with less parameters
theta_guess = np.array([100,0.1])  #Kappa, A
theta_guess = np.concatenate((theta_guess,np.random.randn(5)*1))  #random weight for basis of Kdc kernel
theta_guess = np.concatenate((theta_guess,np.array([10,0.1])))  #tau,dcp_amp
theta_guess = np.concatenate((theta_guess,np.array([0.001,1])))  #amp_h, tau_h
...
#theta_guess = np.concatenate((np.array([K**2,5*0.023]),np.random.randn(5)*1,np.array([scaf,wv_win,scaf_h,h_win])))
###k_, A_, a_N, a_exp, B_N, B_exp = 25, 5, 30, 4, 30, 0.5
res = scipy.optimize.minimize(nLL_hist,theta_guess,args=(data_th,data_dcp,data_dc, data_dth))#,method='Nelder-Mead')
...
plt.figure()
recKdth = theta_fit[9]*np.exp(-K_win/theta_fit[10])  #reconstruct Kdcp kernel
plt.plot(recKdth,'r',label='K_th_fit',linewidth=3)
plt.plot(K_dth,'r-o',label='K_th',linewidth=3)
plt.legend()
...
rescl = 1#np.linalg.norm(K_dc)/np.linalg.norm(recKdc)  #use this before learning the scale factor (it will be exactly the scale factor above)
conv_dc1 = np.dot(recKdc*rescl,dc_n.T) + np.dot(recKdth, data_dth.T)
pxp = sigmoid2(theta_fit[1], recKdc*rescl, dc_n, recKdth, data_dth)
plt.plot(conv_dc1,pxp,'o',linewidth=3,label='inferred',color='r',alpha=0.1)
conv_dc = np.dot(K_dc,dc_n.T) + np.dot(K_dth, data_dth.T)
plt.plot(conv_dc, sigmoid2(5*0.023, K_dc,dc_n, K_dth, data_dth),'o',color='grey',linewidth=5,label='true',alpha=0.1)
plt.xlabel('W*dC+H*dth', fontsize=20)
plt.ylabel('P(turn)', fontsize=20)#,rotation='horizontal') 
#plt.grid(True)
...
Ns = np.array([1,5,50,100,200])#([10,20,40,80,160,320])
#Ns = np.array([5,5,5,5,5])
all_theta_fit = []
MSEs = [] 
for nn in Ns:
    print(nn)
    data_th,data_dcp,data_dc,data_dth = generate_noise(nn)
#    data_th,data_dcp,data_dc,data_dth = generate_taxis(nn)
    res = scipy.optimize.minimize(nLL_hist,theta_guess,args=(data_th,data_dcp,data_dc,data_dth))#,method='Nelder-Mead')
    theta_fit = res.x
    fit_par = theta_fit[2:7]
    recKdc = np.dot(fit_par,RaisedCosine_basis(len(K_dc),len(fit_par)))  #reconstruct Kdc kernel
    recKdcp = theta_fit[7]*np.exp(-K_win/theta_fit[8])  #reconstruct Kdcp kernel
    recKdth = theta_fit[9]*np.exp(-K_win/theta_fit[10]) #reconstruct history kernel
    MSE_dc = np.sum((K_dc-recKdc)**2)
    MSE_dcp = np.sum((K_dcp-recKdcp)**2)
    MSE_dth = np.sum((K_dth-recKdth)**2)
    
    all_theta_fit.append(res.x)  #all theta_fit
    MSEs.append([MSE_dc, MSE_dcp, MSE_dth])  #all MSE measured for two kernels


# %%
pos = np.array([0,1,2,4])
plt.figure()
plt.loglog(Ns[pos],MSEs[pos,:],'-o')
plt.xlabel('N trajectories',fontsize=20)
plt.ylabel('MSE',fontsize=20)
plt.legend({'K_dc','K_dcp','K_h'},fontsize=20)