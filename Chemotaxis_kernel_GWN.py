...
plt.plot(conv_dc1,pxp,'o',linewidth=3,label='inferred',color='k',alpha=0.1)
conv_dc = np.dot(K_dc,dc_n.T)
plt.plot(conv_dc, sigmoid2(5*0.023, K_dc,dc_n),'o',color='grey',linewidth=5,label='true',alpha=0.1)
plt.xlabel('W*dC')
plt.ylabel('P(turn)')#,rotation='horizontal') 
#plt.grid(True)
plt.legend()
