...
dt = 0.1
simtime_len = timelen
learn_every = 3  #effective learning rate
tau = 1  #time scale
...
#2.0*(np.random.rand(nRec2Out,targ_dim)-0.5)*.2

###target pattern
simtime = np.linspace(0,100,simtime_len)
ft = neuraldynamics*1/np.max(neuraldynamics)*.5
#amp = 0.7;
#freq = 1/60;
#rescale = 5.
...
zt = np.zeros((nRec2Out,simtime_len))
x0 = 0.5*np.random.randn(N,1)
z0 = 0.5*np.random.randn(nRec2Out)
...
for t in range(simtime_len-1):
    ti = ti+1
    x = (1.0-dt)*x/tau + M @ (r*dt)/tau #+ wf * (z*dt)
    r = np.tanh(x)
    rt[:,t] = r[:,0]  #xt[:,t] = x[:,0]
    z = wo.T @ r
...
zpt = np.squeeze(zpt)
plt.figure()
plt.plot(zpt.T,label='model')
plt.plot(ft.T,'k--',label='data')
