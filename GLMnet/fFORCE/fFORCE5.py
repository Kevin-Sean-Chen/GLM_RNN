...
with stochastic state transition training!
...
def network_cluster(Ns, ps, gs):
    ...
    p = 0.5
    M_ic = np.random.randn(N_in, N_ch)*.1#0.9/np.sqrt(p*N_in*N_ch)   #test for feed-forward structure
...
def stim_ssm2():
    # parameter
    a = 0.996  # Pr(X(t+1) = 0 | X(t) = 0)
    b = 0.999  # Pr(X(t+1) = 1 | X(t) = 1)
...
for t in range(len(simtime)-1):
    ti = ti+1
    noiset = noise*np.random.randn(N,1)*mask_n  #time varying noise
    ...
    x = (1.0-dt)*x + M @ (r*dt) + wf_s @ stt + noiset#@ z_s #
    r = np.tanh(x)
    rt[:,t] = r[:,0]
    z = wo.T @ r  # linear decoding
    
    ### driven network
    hint = (st[0,t] - st[0,t-1])*5
    x_d = (1.0-dt)*x_d + M_d @ (r_d*dt) + wf @ inpt + wf_s @ stt + wf_h*hint + noiset # z_s #
    r_d = np.tanh(x_d)
    rt_d[:,t] = r_d[:,0]
    ...
    if np.mod(ti, learn_every) == 0:
        ...
        e_J = M[N_in:,N_in:] @ r[N_in:, :] - M_d[N_in:,N_in:] @ r_d[N_in:, :] - wf[N_in:, :] @ inpt 
...
for t in range(lt-1):
    ...
    if t<1000:# and t<700:
        x = (1.0-dt)*x + M @ (r*dt) + 0*wf_s @ fmax + 1*wf_s @ np.array([[0,1]]).T  + 0*noiset*wf_s[:,0][:,None]
...
#fpt = np.squeeze(fpt)
plt.figure()
plt.plot(ft[:].T,'k',linewidth=3,alpha=0.2)
