...
# %% Hidden Markov chain test
def P_state(A,s):
    """
    simple sampling for two states for now
    """
    pos_states = np.array([0,1])
    current_s = np.where(s==pos_states)[0][0]
    state = np.random.choice(states, replace=True, p=A[current_s,:])
...
A = A/A.sum(0)
A = np.array([[0.98,0.01],[0.02,0.99]])
p0 = np.random.rand(states)
p0 = p0/sum(p0)
H = np.zeros(Ts)
ps = np.zeros((states,Ts))
ps[:,0] = p0
for tt in range(0,Ts-1):
    temp = np.dot(A.T,ps[:,tt])
    ps[:,tt+1] = temp/temp.sum()
    H[tt+1] = P_state(A.T,H[tt])#(ps[:,tt])
...
dis2targ = 50
C0 = 0.2*3  #initial concentration
...
T = 1500  #whole duration of steps
dt = 0.6  #seconds
v_m = 0.12  #mm/s
...
ps[:,prehist-1] = p0
H = np.zeros(time.shape)  #hidden state transitioning between different strategies

## with turning (Brownian-like tragectories)
for t in range(prehist,len(time)):
    
    ### compute hidden state
    temp = np.dot(A.T,ps[:,t-1])
    ps[:,t] = temp/temp.sum()
    ...
    dC = dC + np.random.randn(len(dC))*rand
    #dC = np.flip(dC)
    ...
    dth = d_theta(K_dcp_i, -dc_perp, K_i, K_dc_i, dC)
